...
class _MenuPrincipalSesionState extends State<MenuPrincipalSesion> {
  ...
  Future<void> _cargarProcedimientos() async {
    try {
      ...
      setState(() { procedimientos = lista; cargando = false; });
    } catch (e) { setState(() => cargando = false); }
  }

  @override
  Widget build(BuildContext context) {
    if (widget.rol == 'doctor') return const PantallaDoctor();
    if (widget.rol == 'usuario') return const PantallaUsuario();
      ...
      bottomNavigationBar: BottomNavigationBar(
        ...
        onTap: (i) {
          switch (i) {
            ...
            case 1: Navigator.push(context, MaterialPageRoute(builder: (_) => MenuPrincipalSesion(rol: widget.rol))); break;
